...
void stopCommand(const std::map<std::string, std::vector<std::string>> &cmd,
    ...
    std::cout << "Stopping command logic" << std::endl;
    
    // Vérifie que la commande reçue est "stop"
    if (!cmd.count("command") || cmd.at("command").empty() || cmd.at("command")[0] != "stop") {
        std::cerr << "[DEBUG] Command is not 'stop'" << std::endl;
        return;
    }
    
    // Vérifie qu'un service est spécifié dans les arguments
    if (!cmd.count("args") || cmd.at("args").empty()) {
        std::cerr << "No program specified to stop." << std::endl;
        return;
    }
    
    // Le premier argument correspond au nom du service à arrêter.
    ...
    // Récupère la configuration pour déterminer le signal d'arrêt
    ...
    if (signalName == "TERM") signalValue = SIGTERM;
    else if (signalName == "INT") signalValue = SIGINT;
    else if (signalName == "QUIT") signalValue = SIGQUIT;
    else if (signalName == "KILL") signalValue = SIGKILL;
    else if (signalName == "HUP") signalValue = SIGHUP;
    else if (signalName == "USR1") signalValue = SIGUSR1;
    else if (signalName == "USR2") signalValue = SIGUSR2;
    ...
    std::vector<pid_t> pidsToStop;
    {
        ...
        pidsToStop = runningServices[requestedProgram].pids;
    }
    ...
    for (pid_t pid : pidsToStop) {
        ...
        if (kill(pid, signalValue) != 0) {
            std::cerr << "Failed to send signal to PID " << pid << ": " << strerror(errno) << std::endl;
        }
    }
    
    // Attendre que les processus se terminent (avec timeout)
    int stoptime = config.getStoptime();
    std::cout << "Waiting up to " << stoptime << " seconds for processes to terminate..." << std::endl;
    ...
    // Si des processus sont toujours en vie après le timeout, utiliser SIGKILL
    if (!pidsToStop.empty()) {
